...
int begin_flag = 1, weather_flag = 1, break_flag = 0,sleep_flag = 0;
...
String UserKey = "xxxxxxxxxxxx";	//私钥获取地址 https://dev.heweather.com/docs/start/get-api-key
...
void loop() {
  ...
  hours = timeClient.getHours();
	...
	{
		if (minu < 10)			minu_string = "0" + String(minu);
		else			minu_string = String(minu);
		if (hours < 10)			hours_string = "0" + String(hours);
		else			hours_string = String(hours);
		time_string = hours_string + ":" + minu_string;
		for (int i = 0; i <= LineWidth - 1; i++)			tft.drawFastVLine(78 + i, 0, 53, TFT_WHITE);
		printString(83, 13, 2, time_string);
		update_week(week + 1, 56 + 8, 59);
		update_week(week + 2, 112 + 8, 59);
		update_week(week, 95, 38);
	...
	if( ((minu == 10) ||(minu == 40)) && (weather_flag == 1) && (sleep_flag == 0))		//更新天气
	{
		#if Debug
